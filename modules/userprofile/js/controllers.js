...
angular.module('cartApp.userprofile.controller', []).controller('UserprofileController', ['$scope','loginFactory','signupFactory','$rootScope','productsFactory',function($scope,loginFactory,signupFactory,$rootScope,productsFactory){
	...
	$scope.grandTotal = productsFactory.getGrandTotal();
	console.log('get Grand Total: '+ $scope.grandTotal);
	...
	$scope.fetchedProdata = function(){
		productsObj.fetchProdataFromdb()
			.then(function(data){
				$scope.userProdata = data;
				console.log('userProdata: '+ $scope.userProdata);
			},function(){
				console.log('data doesn\'t retrieved');
			});
	}
